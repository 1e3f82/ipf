...
import static org.openehealth.ipf.commons.xml.XqjTransmogrifier.RESOURCE_LOCATION
...
class TransmogrifierRouteBuilder extends SpringRouteBuilder {
    ...
    void configure() {
        ...
        from('direct:input28') // passing classpath parameters
                .transmogrify().xquery()
                .staticParams('xquery/extract-map-resource.xq', [id:'someid', mapfile:'xquery/mapping.xml'])
                .to('mock:output')

        from('direct:input29') // passing classpath parameters
                .transmogrify().xquery()
        ...
        from('direct:input30') // passing classpath parameters
                .setHeader('someid', constant('headerId'))
                .transmogrify().xquery().staticParams('xquery/extract.xq').params().header('someid')
                .to('mock:output')
    }
